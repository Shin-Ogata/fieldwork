/*!
 * @cdp/framework-worker 0.9.5
 *   worker library collection
 */

(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@cdp/framework-core')) :
    typeof define === 'function' && define.amd ? define(['exports', '@cdp/framework-core'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.CDP = global.CDP || {}, global.CDP));
}(this, (function (exports, frameworkCore) { 'use strict';
    ...
    /** @internal */ const btoa = frameworkCore.safe(globalThis.btoa);
    /** @internal */ const atob = frameworkCore.safe(globalThis.atob);
    /** @internal */ const Blob = frameworkCore.safe(globalThis.Blob);
    /** @internal */ const FileReader = frameworkCore.safe(globalThis.FileReader);
    /** @internal */ const URL = frameworkCore.safe(globalThis.URL);
    ...
    function exec(methodName, args, options) {
        ...
        token && frameworkCore.verify('instanceOf', frameworkCore.CancelToken, token);
        onprogress && frameworkCore.verify('typeOf', 'function', onprogress);
    ...
    async function serialize(data, options) {
        ...
        await frameworkCore.checkCanceled(cancel);
        if (null == data) {
            return String(data);
        }
            ...
            return frameworkCore.fromTypedData(data);
        }
    }
    async function deserialize(value, options) {
        const { dataType, cancel } = options || {};
        await frameworkCore.checkCanceled(cancel);
        const data = frameworkCore.restoreNil(frameworkCore.toTypedData(value));
        switch (dataType) {
            case 'string':
                return frameworkCore.fromTypedData(data);
            case 'number':
            ...
            case 'object':
                return Object(data);
            case 'buffer':
                return dataURLToBuffer(frameworkCore.fromTypedData(data));
            case 'binary':
                return dataURLToBinary(frameworkCore.fromTypedData(data));
            case 'blob':
                return dataURLToBlob(frameworkCore.fromTypedData(data));
            default:
                return data;
        }
    }
    ...
    /** @internal */ const _FormData = frameworkCore.safe(globalThis.FormData);
    /** @internal */ const _Headers = frameworkCore.safe(globalThis.Headers);
    /** @internal */ const _AbortController = frameworkCore.safe(globalThis.AbortController);
    /** @internal */ const _URLSearchParams = frameworkCore.safe(globalThis.URLSearchParams);
    /** @internal */ const _XMLHttpRequest = frameworkCore.safe(globalThis.XMLHttpRequest);
    /** @internal */ const _fetch = frameworkCore.safe(globalThis.fetch);
    ...
    const settings = {
        ...
        set timeout(value) {
            _timeout = (frameworkCore.isNumber(value) && 0 <= value) ? value : undefined;
        },
    };
    ...
    function ensureParamValue(prop) {
        const value = frameworkCore.isFunction(prop) ? prop() : prop;
    ...
    async function ajax(url, options) {
        ...
        const source = frameworkCore.CancelToken.source(originalToken);
        const { token } = source;
        token.register(abort);
        // timeout
        if (timeout) {
            setTimeout(() => source.cancel(frameworkCore.makeResult(frameworkCore.RESULT_CODE.ERROR_AJAX_TIMEOUT, 'request timeout')), timeout);
        }
        // normalize
        ...
        if ('response' === dataType) {
            ...
            throw frameworkCore.makeResult(frameworkCore.RESULT_CODE.ERROR_AJAX_RESPONSE, response.statusText, response);
        }
        else {
    ...
    function resource(url, dataType, data) {
        ...
        if (!(200 <= xhr.status && xhr.status < 300)) {
            throw frameworkCore.makeResult(frameworkCore.RESULT_CODE.ERROR_AJAX_RESPONSE, xhr.statusText, xhr);
        }
    ...
    /** @internal */ const URL$1 = frameworkCore.safe(globalThis.URL);
    /** @internal */ const Worker = frameworkCore.safe(globalThis.Worker);
    /** @internal */ const Blob$1 = frameworkCore.safe(globalThis.Blob);
    /** @internal */
    function createWorkerContext(src) {
        if (!(frameworkCore.isFunction(src) || frameworkCore.isString(src))) {
            throw new TypeError(`${frameworkCore.className(src)} is not a function or string.`);
        }
        return URL$1.createObjectURL(new Blob$1([frameworkCore.isFunction(src) ? `(${src.toString()})(self);` : src], { type: 'application/javascript' }));
    }
    /**
    ...
    function thread(executor, options) {
        ...
        originalToken?.register(abort);
        const { token } = frameworkCore.CancelToken.source(originalToken);
        return new Promise((resolve, reject) => {
            worker.onerror = ev => {
                ev.preventDefault();
                reject(ev);
