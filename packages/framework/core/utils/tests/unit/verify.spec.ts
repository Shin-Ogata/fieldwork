...
describe('utils/types spec', () => {
    ...
    it(`check verify('notNil')`, () => {
        expect(() => verify('notNil', undefined)).toThrow(new TypeError('Undefined is not a valid value.'));
        expect(() => verify('notNil', null)).toThrow(new TypeError('Null is not a valid value.'));
        expect(() => verify('notNil', void 0)).toThrow(new TypeError('Undefined is not a valid value.'));
        expect(() => verify('notNil', undefined, '`undefined` input')).toThrow(new TypeError('`undefined` input'));
        expect(() => verify('notNil', null, null)).toThrow(new TypeError('Null is not a valid value.'));
        expect(() => verify('notNil', '')).not.toThrow();
        expect(() => verify('notNil', false)).not.toThrow();
        expect(() => verify('notNil', 0)).not.toThrow();
        expect(() => verify('notNil', NaN)).not.toThrow();
        expect(() => verify('notNil', {})).not.toThrow();
        expect(() => verify('notNil', [])).not.toThrow();
        expect(() => verify('notNil', _func)).not.toThrow();
        expect(() => verify('notNil', _symbol)).not.toThrow();
        expect(() => verify('notNil', TypeClass)).not.toThrow();
        expect(() => verify('notNil', _classInst)).not.toThrow();
    });
