...
(function (global, factory) {
    ...
    const toRouteContextParameters = (routes) => {
        ...
        return flatten('', coreUtils.isArray(routes) ? routes : routes ? [routes] : [])
            .map((seed) => {
            seed.regexp = extensionPath2regexp.path2regexp.pathToRegexp(seed.path);
            seed.paramKeys = seed.regexp.keys.filter(k => coreUtils.isString(k.name)).map(k => k.name);
            return seed;
        });
    };
    //__________________________________________________________________________________________________//
