...
`));
    ...
    describe('utility', () => {
        it('coverage DOM#reflow(), DOM#repaint()', () => {
            prepareTestElements(testee(`<div id="d1" class="test-dom" style="position: absolute; width: 10px; height: 10px;"></div>`));
            const $dom = $('.test-dom');
            expect(() => $dom.reflow()).not.toThrow();
            expect(() => $dom.repaint()).not.toThrow();
            const $window = $(window);
            expect(() => $window.reflow()).not.toThrow();
            expect(() => $window.repaint()).not.toThrow();
            const $document = $(document);
            expect(() => $document.reflow()).not.toThrow();
            expect(() => $document.repaint()).not.toThrow();
        });
    });
});
